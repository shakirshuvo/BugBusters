...
public class AllEmployeeAPI {
    ...
    public void badCallAllEmployeeResources(){
        ...
        int statusCode = 0;
        try {
            response = given().when().get(baseUrl+"api/AllEmployeeResources809898");
    ...
    public void serverErrorCallAllEmployeeResources(){
        ...
        int statusCode = 0;
        try {
            response = given().when().get(baseUrl+"/api/AllEmployeeResources").then().statusCode(500).extract().response();
            statusCode = response.getStatusCode();
        }catch(Exception ex){
        }
